...
FSM.prototype.log = function () {
	arguments[0] = '*FSM* : ' + arguments[0];
	this._app.log.apply(this, arguments);
};

FSM.prototype.name = function (val) {
	if (val !== undefined) {
		this._name = val;
		return this;
	}
...
/**
 * Gets / sets the debug flag. If set to true will enable console self.logging
 ...
 *     fsm.defineState('idle', {
	 ...
	 *             console.self.log('entered idle state');
	 *             completeCallback();
	 *         },
	 *         exit: function (data, completeCallback) {
	 *             console.self.log('exited idle state');
	 *             completeCallback();
	 *         }
	 *     });
 * @returns {FSM}
 ...
 *     fsm.defineState('moving', {
	 *         enter: function (data, completeCallback) {
	 *             console.self.log('entered moving state');
	 *             completeCallback();
	 *         },
	 *         exit: function (data, completeCallback) {
	 *             console.self.log('exited moving state');
	 *             completeCallback();
	 *         }
	 *     });
 *
 ...
 *     fsm.enterState('moving', 'notOk', function (err, data) {
	 ...
	 *             console.self.log('State changed!', fsm.currentStateName());
	 *         } else {
	 *             // There was an error, the state did not change
	 *             console.self.log('State did NOT change!', fsm.currentStateName());
	 *         }
	 *     });
 *
...
FSM.prototype.defineTransition = function (fromState, toState, transitionCheck) {
	if (fromState && toState && transitionCheck) {
		if (!this._states[fromState]) {
			self.log('fromState "' + fromState + '" specified is not defined as a state!', 'error');
		}

		if (!this._states[toState]) {
			self.log('toState "' + toState + '" specified is not defined as a state!', 'error');
		}
...
FSM.prototype.initialState = function (stateName, data, callback) {
	...
	if (this._debug) { self.log('Entering initial state: ' + stateName); }
	...
	if (self._transitions[self._currentStateName] && self._transitions[self._currentStateName][newStateName]) {
		...
		self._transitions[self._currentStateName][newStateName](data, function (err) {
			if (!err) {
				...
				self.log('Cannot transition from "' + self._currentStateName + '" to "' + newStateName + '" states.', 'warning');
			}
		});
	} else {
